...
global.menuImage     = "https://files.catbox.moe/zp471g.jpeg";
...
// ─── FITUR AI (jawab pertanyaan apa saja) ──────────────────────────────────
// Pakai Google Gemini API (ada free tier). Daftar & ambil API key gratis di:
// https://aistudio.google.com/apikey lalu tempel di bawah ini.
global.geminiApiKey  = "AQ.Ab8RN6Ic0bhMKgk2oyu3mt9jxiBV03PXxb4lbyE2SZ5lv16Vsg"; // ← API key Gemini kamu
global.geminiModel   = "gemini-2.5-flash";             // ← Model Gemini yang dipakai (2.0-flash sudah dimatikan Google per Juni 2026)
...
// ─── AI: jawab pertanyaan apa saja (Google Gemini) ─────────────────────────
async function handleAI(sock, msg, jid, pertanyaan) {
  if (!pertanyaan || !pertanyaan.trim()) {
    return reply(sock, msg, `🤖 *AI ASSISTANT*\n\nCara pakai:\n${global.prefix}ai <pertanyaan kamu>\n\nContoh:\n${global.prefix}ai apa itu lubang hitam?`);
  }

  if (!global.geminiApiKey) {
    return reply(sock, msg, `⚠️ Fitur AI belum aktif.\n\nOwner bot perlu isi *geminiApiKey* dulu di config script (ambil API key gratis di https://aistudio.google.com/apikey).`);
  }

  const replyWait = makeReplyWait(sock, jid, msg);
  await replyWait();

  try {
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${global.geminiModel}:generateContent?key=${global.geminiApiKey}`;
    ...
    const jawaban =
      ...
      "❌ Maaf, AI tidak memberikan jawaban. Coba pertanyaan lain ya.";

    await reply(sock, msg, `🤖 *AI ASSISTANT*\n\n${jawaban.trim()}`);
  } catch (e) {
    const status = e.response?.status;
    const apiMsg = e.response?.data?.error?.message;
    console.error("handleAI error:", status, apiMsg || e.message);

    let pesan = "❌ Gagal menghubungi AI. Coba lagi beberapa saat lagi ya.";
    if (status === 400 || status === 401 || status === 403) {
      pesan = `❌ API key Gemini ditolak (${status}). Cek lagi apakah key masih aktif/valid di https://aistudio.google.com/apikey.`;
    } else if (status === 404) {
      pesan = `❌ Model "${global.geminiModel}" tidak ditemukan/sudah dihentikan Google. Owner perlu ganti ke model yang masih aktif (mis. gemini-2.5-flash).`;
    } else if (status === 429) {
      pesan = "❌ Kuota gratis AI hari ini sudah habis / kena rate limit. Coba lagi nanti ya.";
    }
    await reply(sock, msg, pesan);
  }
}
...
async function handleMessage(sock, msg) {
  try {
    ...
    switch (command) {
      ...
      case "ai": case "ask": case "tanya": case "chatgpt": case "gpt":
        await handleAI(sock, msg, jid, args.join(" ")); break;
